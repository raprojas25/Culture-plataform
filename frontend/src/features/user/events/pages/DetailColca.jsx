...
const EventDetailsColca = () => {
  const data = {
    ...
    address: "Cruz del Cóndor, Valle del Colca",
    district_id: 3,
  ...
  useEffect(() => {
    const fetchEvent = async () => {
      try {
        ...
        // Datos de ejemplo inspirados en el Valle del Colca
        setTimeout(() => {
          setEvent(data);
          setLoading(false);
        }, 1000);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-amber-50 to-white dark:from-gray-900 dark:to-gray-800 flex items-center justify-center">
        <div className="text-center max-w-md p-8 bg-white dark:bg-gray-800 rounded-2xl shadow-xl">
          ...
          <button
            onClick={() => navigate("/")}
            className="px-6 py-3 bg-amber-500 hover:bg-amber-600 text-white rounded-lg font-semibold transition duration-300"
          >
            Volver al inicio
          </button>
        </div>
      ...
      <main className="container mx-auto px-4 py-8 -mt-20 relative z-30">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            {/* Tabs */}
            <Tab.Group>
              <Tab.List className="flex space-x-2 bg-white dark:bg-gray-800 p-2 rounded-2xl shadow-lg border border-amber-100 dark:border-gray-700 overflow-x-auto">
                {tabs.map(({ name, icon: Icon }) => (
                  <Tab
                    key={name}
                    className={({ selected }) =>
                      `flex-1 py-3 px-4 rounded-xl text-center font-semibold transition-all duration-300 flex items-center justify-center space-x-2 focus:outline-none outline-none ${
                        selected
                    ...
                    <div className="space-y-6">
                      {event.details
                        ...
                        .map((detail, index) => (
                          <div
                            key={index}
                            className="flex items-center space-x-4 p-4 rounded-xl border-l-4 border-amber-500 bg-amber-50/50 dark:bg-gray-600/50"
                          >
...
export default EventDetailsColca;
