...
const eventSchema = z.object({
  ...
  start_datetime: z.string().regex(/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/, {
    message: "Formato de fecha inválido. Use YYYY-MM-DD HH:mm:ss",
  }),

  end_datetime: z.string().regex(/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/),
...
const EventForm = () => {
  ...
  const {
    ...
    defaultValues: {
      price_type: "free",
      featured_level: 0,
      details: [{ section: "schedule", title: "", content: "", order: 0 }],
      images: [],
      contact: {
        name: "",
        phone: "",
          ...
          <div className="mt-4 text-sm text-gray-500 text-center">
            {Object.keys(errors).length > 0 && (
              <p className="text-red-600 mb-2">
                Por favor, corrige los {Object.keys(errors).length} errores en
                el formulario
              </p>
            )}
            <p>
