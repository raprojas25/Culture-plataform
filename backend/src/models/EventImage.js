...
export class EventImage {
  ...
  static async createMultiple(images) {
    ...
    const params = images.flatMap(img => [
      img.event_id, img.image_url, img.alt_text || '', img.order_index || 0
    ]);
  ...
  static async delete(id) {
    const result = await query('DELETE FROM event_images WHERE id = \$1 RETURNING *', [id]);
    return result.rows[0];
  }
  ...
  static async setMainImage(eventId, imageId) {
    ...
    await query(
      'UPDATE event_images SET is_main = false WHERE event_id = \$1',
      [eventId]
    );
